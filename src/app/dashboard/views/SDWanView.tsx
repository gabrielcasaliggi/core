...
import {
  ...
  AlertTriangle, CheckCircle2, Loader2, Zap, RefreshCw,
} from "lucide-react";
...
function useSiteName(siteId: string, fallback: Record<string, string>): string {
  const { snapshot } = useNetwork();
  const site = snapshot.sites.find(s => s.id === siteId);
  return site?.name ?? fallback[siteId] ?? siteId;
}
...
export default function SDWanView() {
  const { wanInterfaces, forceFailover } = useNetwork();
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ background: "#000814" }}>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-6">
        {Object.entries(bySite).map(([siteId, ifaces]) => (
          <SiteSection key={siteId} siteId={siteId} ifaces={ifaces}
            loadingIface={loadingIface} successIface={successIface}
...
function SiteSection({ siteId, ifaces, loadingIface, successIface, onFailover }: {
  siteId: string;
  ifaces: WanInterface[];
  loadingIface: string | null;
  successIface: string | null;
  onFailover: (iface: WanInterface) => void;
}) {
  const siteName = useSiteName(siteId, SITE_NAMES);
    ...
    <section>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="data-value text-[11px] font-semibold tracking-wider"
          style={{ color: "#818cf8" }}>{siteName}</span>
